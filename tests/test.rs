...
#[test]
fn test_array() {
    let array = QuoteIter([0u8; 40]);
    let _ = quote!(#array);

    let ref_array = QuoteIter(&[0u8; 40]);
...
fn test_inner_block_comment() {
    ...
    assert_eq!("#! [doc = r\" doc \"]", tokens.to_string());
}
...
fn test_type_inference_for_span() {
    ...
    let delim_span = Group::new(Delimiter::Parenthesis, TokenStream::new())
        .delim_span()
        .join();
    ...
    if false {
        let proc_macro_span = proc_macro::Span::call_site().into();
        let _ = quote_spanned!(proc_macro_span=> ...);
    }
}
